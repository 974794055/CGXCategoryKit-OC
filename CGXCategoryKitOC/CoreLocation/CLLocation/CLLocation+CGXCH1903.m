...
@implementation CLLocation (CGXCH1903)


- (id)initWithCH1903x:(double)x y:(double)y
{
    self = [self initWithLatitude:[CLLocation gx_CHtoWGSlatWithX:x y:y] longitude:[CLLocation gx_CHtoWGSlongWithX:x y:y]];
    if (self != nil) {
...
- (double)gx_CH1903Y {
    return [CLLocation gx_WGStoCHyWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}

- (double)gx_CH1903X {
    return [CLLocation gx_WGStoCHxWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}
...
+ (double)gx_WGStoCHyWithLatitude:(double)lat longitude:(double)lng {
    ...
    lat = [CLLocation gx_decToSex:lat];
    lng = [CLLocation gx_decToSex:lng];
    
    // Converts degrees to seconds (sex)
    lat = [CLLocation gx_degToSec:lat];
    lng = [CLLocation gx_degToSec:lng];

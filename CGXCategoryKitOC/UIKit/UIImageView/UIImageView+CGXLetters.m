...
static const CGFloat gx_FontResizingProportion = 0.42f;

@interface UIImageView (CGXLettersPrivate)

- (UIImage *)gx_imageSnapshotFromText:(NSString *)text backgroundColor:(UIColor *)color circular:(BOOL)isCircular textAttributes:(NSDictionary *)attributes;

@end

@implementation UIImageView (CGXLetters)

- (void)gx_imageWithString:(NSString *)string {
    [self gx_imageWithString:string color:nil circular:NO textAttributes:nil];
}

- (void)gx_imageWithString:(NSString *)string color:(UIColor *)color {
    [self gx_imageWithString:string color:color circular:NO textAttributes:nil];
}

- (void)gx_imageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular {
    [self gx_imageWithString:string color:color circular:isCircular textAttributes:nil];
}

- (void)gx_imageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular fontName:(NSString *)fontName {
    [self gx_imageWithString:string color:color circular:isCircular textAttributes:@{
                                                                                     NSFontAttributeName:[self gx_fontForFontName:fontName],
...
- (void)gx_imageWithString:(NSString *)string color:(UIColor *)color circular:(BOOL)isCircular textAttributes:(NSDictionary *)textAttributes {
    if (!textAttributes) {
        textAttributes = @{
                           NSFontAttributeName: [self gx_fontForFontName:nil],
                           NSForegroundColorAttributeName: [UIColor whiteColor]
                           };
    }
    ...
    UIColor *backgroundColor = color ? color : [self gx_letter_randomColor];

    self.image = [self gx_imageSnapshotFromText:[displayString uppercaseString] backgroundColor:backgroundColor circular:isCircular textAttributes:textAttributes];
}

#pragma mark - Helpers

- (UIFont *)gx_fontForFontName:(NSString *)fontName {
    
    CGFloat fontSize = CGRectGetWidth(self.bounds) * gx_FontResizingProportion;
    if (fontName) {
...
- (UIImage *)gx_imageSnapshotFromText:(NSString *)text backgroundColor:(UIColor *)color circular:(BOOL)isCircular textAttributes:(NSDictionary *)textAttributes {

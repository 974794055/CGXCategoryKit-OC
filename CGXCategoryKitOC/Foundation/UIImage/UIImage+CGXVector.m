...
@implementation UIImage (CGXVector)
+ (NSCache *)gx_cache{
...
+ (UIImage *)gx_iconWithFont:(UIFont *)font named:(NSString *)iconNamed withTintColor:(UIColor *)tintColor clipToBounds:(BOOL)clipToBounds forSize:(CGFloat)fontSize{
    ...
    UIImage *image = [[self gx_cache] objectForKey:identifier];
    if(image == nil){
        ...
        if(!CGSizeEqualToSize(CGSizeZero, imageSize)){
            ...
            [[self gx_cache] setObject:image forKey:identifier];
        }
    }
    return image;
}
...
+ (UIImage *)gx_imageWithPDFNamed:(NSString *)pdfNamed withTintColor:(UIColor *)tintColor forHeight:(CGFloat)height{
    ...
    return [self gx_imageWithPDFFile:pdfFile withTintColor:tintColor forSize:CGSizeMake(MAXFLOAT, height)];
}

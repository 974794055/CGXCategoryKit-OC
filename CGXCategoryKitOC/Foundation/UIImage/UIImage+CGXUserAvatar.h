...
///字符显示模式
typedef NS_ENUM(NSUInteger, CGXAvatarImageType)
{
    ///全部显示(字符长度尽量短,否则可能出现文字超出图片边界的情况)
    CGXAvatarImageTypeShowAll             = 0,
    ///显示第一位字符
    CGXAvatarImageTypeFirstCharacter      = 1,
    ///显示最后一位字符
    CGXAvatarImageTypeLastCharacter       = 2,
};

@interface CGXUserAvatarOptional : NSObject
...
@property (nonatomic, assign) CGXAvatarImageType avatarImageType;

@end

@interface UIImage (CGXUserAvatar)

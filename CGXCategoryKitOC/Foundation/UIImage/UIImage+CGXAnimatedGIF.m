...
@implementation UIImage (CGXAnimatedGIF)
...
+ (UIImage *)gx_animatedImageWithAnimatedGIFData:(NSData *)data {
    return animatedImageWithAnimatedGIFReleasingImageSource(CGImageSourceCreateWithData(toCF data, NULL));
}

+ (UIImage *)gx_animatedImageWithAnimatedGIFURL:(NSURL *)url {
...
+ (UIImage *)gx_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self gx_frameDurationAtIndex:i source:source];
...
+ (float)gx_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)gx_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage gx_animatedGIFWithData:data];
        }

...
- (NSData *)gx_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self gx_isGzippedData]) {
        return self;
    }
...
- (NSData *)gx_gzippedData
{
    return [self gx_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)gx_gunzippedData
{
    if (self.length == 0 || ![self gx_isGzippedData])
    {
        return self;
...
{
    if (self.length == 0 || [self gx_isZlibbedData])
    {
        return self;
    }
...
{
    return [self gx_zlibbedDataWithCompressionLevel:-1.0f];
}

...
@implementation NSObject (CGXObject)
...
{
    return [[self class] gx_propertyKeys];
}
...
-(NSArray*)gx_methodListInfo
{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] gx_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] gx_decodeType:returnType];
        NSString *encodeString = [[self class] gx_decodeType:encoding];
...
+ (NSArray *)gx_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] gx_decodeType:ivar_getTypeEncoding(ivars[i])];
...
-(NSDictionary *)gx_protocolList{
    return [[self class]gx_protocolList];
}
+ (NSDictionary *)gx_protocolList
{
...
{
    return [[self class] gx_propertiesInfo];
}
/**
...
+ (NSArray *)gx_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self gx_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)gx_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] gx_propertiesInfo];
...
+ (NSString *)gx_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString gx_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
...
-(void)gx_modelWithDictionary:(NSDictionary *)dict{
    //得到本模型的所有属性
...
#pragma mark -- help
+ (NSDictionary *)gx_dictionaryWithProperty:(objc_property_t)property
{
    NSMutableDictionary *result = [NSMutableDictionary dictionary];

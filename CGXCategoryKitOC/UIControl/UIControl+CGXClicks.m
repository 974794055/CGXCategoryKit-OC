...
@implementation UIControl (CGXClicks)
...
- (void)my_sendAction:(SEL)action to:(id)target forEvent:(UIEvent *)event {
    ...
    if ([self isKindOfClass:[UIButton class]]) {
        ...
        //1. 按钮点击间隔事件
        self.gx_clickDurationTime = self.gx_clickDurationTime == 0 ? defaultDuration : self.gx_clickDurationTime;
        
        //2. 是否忽略按钮点击事件
        if (_isIgnoreEvent) {
            //2.1 忽略按钮事件
            return;
        } else if(self.gx_clickDurationTime > 0) {
            //2.2 不忽略按钮事件
            ...
            // 间隔事件后，执行按钮事件
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.gx_clickDurationTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                resetState();
            });
...
- (void)setGx_clickDurationTime:(NSTimeInterval)gx_clickDurationTime
{
    objc_setAssociatedObject(self, @selector(gx_clickDurationTime), @(gx_clickDurationTime), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSTimeInterval)gx_clickDurationTime {
    return [objc_getAssociatedObject(self, @selector(gx_clickDurationTime)) doubleValue];
}
